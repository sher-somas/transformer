...
def get_or_build_tokenizer(config, dataset, language):
    ...
    min_frequency --> a word should exist at least twice in the corpus
    """
...
def get_dataset(config):
    ...
    max_len_src_sentence = 0 
    max_len_tgt_sentence = 0
    ...
    for item in dataset_raw:
        ...
        max_len_src_sentence = max(max_len_src_sentence, len(src_ids))
        max_len_tgt_sentence = max(max_len_tgt_sentence, len(tgt_ids))

    print(f"max length of the source sentence: {max_len_src_sentence}")
    print(f"max length of the target sentence: {max_len_tgt_sentence}")
...
def get_model(config, src_vocab_len, tgt_vocab_len):
    ...
    model = build_transformer(src_vocab_size=src_vocab_len, tgt_vocab_size=tgt_vocab_len, src_seq_len=config['seq_len'],
